if __name__ == "__main__":
    ...
    fZZ=ROOT.TFile("output_SS_"+options.year+"/ZZ4L.root","r")
    fWZ=ROOT.TFile("output_SS_"+options.year+"/WZ3LNu.root","r")
    fZG=ROOT.TFile("output_SS_"+options.year+"/ZG.root","r")
    fTTZ=ROOT.TFile("output_SS_"+options.year+"/TTZ.root","r")
    fTTW=ROOT.TFile("output_SS_"+options.year+"/TTW.root","r")
    fWWW=ROOT.TFile("output_SS_"+options.year+"/WWW.root","r")
    fData=ROOT.TFile("output_SS_"+options.year+"/Data.root","r")
    fout=ROOT.TFile("output_SS_"+options.year+"/nonprompt.root","recreate")
    ...
    for j in range(0,ncat):
	  ...
	  hPF=fData.Get("em_"+str(j)+"_anti1/data_obs"+postfix).Clone()
          ...
          hPF.Add(fZG.Get("em_"+str(j)+"_anti1/VG"),-1)
          ...
          hFP.Add(fZG.Get("em_"+str(j)+"_anti2/VG"),-1)
          ...
          hFF.Add(fZG.Get("em_"+str(j)+"_anti0/VG"),-1)
          ...
          hPFR.Add(fZG.Get("emR_"+str(j)+"_anti1/VG"),-1)
          ...
          hFPR.Add(fZG.Get("emR_"+str(j)+"_anti2/VG"),-1)
          ...
          hFFR.Add(fZG.Get("emR_"+str(j)+"_anti0/VG"),-1)
